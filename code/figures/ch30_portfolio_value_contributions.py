...
Portfolio value contributions by position.
...
def main() -> None:
    ...
    values = np.array([p["value"] for p in report["positions"]], dtype=float)

    fig, ax = plt.subplots(figsize=(7.8, 4.2))
    colors = ["C0" if v >= 0 else "C3" for v in values]
    ax.bar(names, values, color=colors)
    ax.axhline(0.0, color="black", linewidth=0.8)
    ax.set_title("Portfolio Value Contributions")
    ax.set_ylabel("Value")
    ...
    outfile = figures_dir / "ch30_portfolio_value_contributions.png"
