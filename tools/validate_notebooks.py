...
def execute_notebook(path: Path, *, timeout: int) -> tuple[bool, str | Exception, float]:
    ...
    runner = (
        ...
        "# Validation must exercise each benchmark, not reproduce timeit's\n"
        "# full interactive sampling protocol.  One timed execution keeps\n"
        "# computationally intensive performance notebooks within the outer\n"
        "# notebook timeout while still running every benchmarked expression.\n"
        "for cell in nb.cells:\n"
        "    if cell.cell_type != 'code':\n"
        "        continue\n"
        "    lines = cell.source.splitlines()\n"
        "    cell.source = '\\n'.join(\n"
        "        line.replace('%timeit ', '%timeit -r 1 -n 1 ', 1)\n"
        "        if line.lstrip().startswith('%timeit ') else line\n"
        "        for line in lines\n"
        "    )\n"
